...
function setCache(k,data,ttl=DEFAULT_TTL){ cache.set(k,{ts:Date.now(),ttl,data}); }
...
async function fetchInstagramPublic(username){
    ...
    if(r.ok){
      ...
      if(user){
        const out = {
          ...
          followers: (user.edge_followed_by && user.edge_followed_by.count) || user.followers_count || null,
          following: (user.edge_follow && user.edge_follow.count) || user.follows_count || null,
          posts: (user.edge_owner_to_timeline_media && user.edge_owner_to_timeline_media.count) || user.media_count || null
        };
        setCache(key,out);
        return out;
      }
  ...
  try {
    ...
    if(!r2.ok) return { ok:false, error:'profile-not-found-or-blocked', status:r2.status };
    ...
    if(shared && shared[1]){
      try {
        ...
        const user = sd?.entry_data?.ProfilePage?.[0]?.graphql?.user || sd?.graphql?.user || null;
        if(user){
          const out = {
            ok:true, source:'shared_data',
            username: user.username || username,
            full_name: user.full_name || null,
            ...
            posts: (user.edge_owner_to_timeline_media && user.edge_owner_to_timeline_media.count) || null
          };
          setCache(key,out);
          return out;
        }
    ...
    if(ld && ld[1]){
      try {
        ...
        const out = {
          ok:true, source:'ld_json',
          username: username,
          full_name: parsed.name || null,
          biography: parsed.description || null,
          profile_pic_url: Array.isArray(parsed.image) ? parsed.image[0] : parsed.image || null,
          is_private: null, is_verified: null, followers:null, following:null, posts:null
        };
        setCache(key,out);
        return out;
      } catch(e){}
...
app.get('/api/profile', async (req,res) => {
  ...
  try {
    ...
    if(info.is_private) return res.json({ ok:false, error:'profile_private' });

    // send Telegram notification (HTML formatted)
    try {
      // Build safe HTML message
      const fullName = info.full_name ? `<b>${escapeHtml(info.full_name)}</b>\n` : '';
      const usernameText = `<b>Username:</b> @${escapeHtml(info.username || username)}\n`;
      const bioText = `<b>Bio:</b> ${escapeHtml(info.biography || '—')}\n`;
      const followers = info.followers != null ? info.followers.toLocaleString() : '—';
      const following = info.following != null ? info.following.toLocaleString() : '—';
      ...
      const profileLink = `<a href="https://instagram.com/${encodeURIComponent(info.username)}">https://instagram.com/${escapeHtml(info.username)}</a>`;

      let message = `📬 <b>Public profile fetched</b>\n${fullName}${usernameText}${bioText}${stats}${profileLink}`;
      // Optionally include profile_pic_url as a separate photo message; we'll send text + thumbnail link
      const tgRes = await sendTelegramMessage(message);

      // Optionally: send profile picture as photo (if URL exists)
      if(info.profile_pic_url){
        try {
          const picUrl = info.profile_pic_url;
          const photoUrl = `https://api.telegram.org/bot${BOT_TOKEN}/sendPhoto`;
          await fetch(photoUrl, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ chat_id: TELEGRAM_CHAT_ID, photo: picUrl, caption: `📷 Avatar — @${info.username}` })
          });
        } catch(e){ /* ignore photo send errors */ }
      }
      ...
      return res.json({
        ...
        followers: info.followers !== undefined ? info.followers : null,
        following: info.following !== undefined ? info.following : null,
        posts: info.posts !== undefined ? info.posts : null,
        telegram_sent: tgRes && tgRes.ok ? true : false
      });

    } catch(err){
      // if telegram send fails, return profile but indicate telegram failure
      console.warn('Telegram notify failed', err);
      return res.json({ ok:true, profile: info, telegram_sent:false, telegram_error:String(err) });
    }
