...
<canvas id="particles"></canvas>  <div class="card">  
  <div class="logo"></div>  
  <h1>PRIVATE ACCOUNT VIEWER</h1>  
  <div class="sub"> Enter your username & the account you want to search</div>  <label>Your Username</label>
...
<button class="btn" id="searchBtn"> Search</button>
  ...
  <div class="status" id="status">Waiting…</div>    <div class="result" id="result">  
...
/* demo search flow */  
const btn=document.getElementById("searchBtn"),status=document.getElementById("status"),res=document.getElementById("result");  
btn.onclick=()=>{  
  const u=document.getElementById("target").value.trim();if(!u){alert("Enter a username!");return;}  
  res.style.display="none";status.textContent=" Searching…";  
  setTimeout(()=>{status.textContent=" Getting info…";setTimeout(()=>{status.textContent=" Found public info";show(u);},1200);},1200);  
};  
function show(u){  
  document.getElementById("handle").textContent="@"+u.replace(/^@/,"");  
  document.getElementById("bio").textContent="This is a demo public bio for "+u+" ";  
  document.getElementById("f1").textContent=Math.floor(Math.random()*1e5);  
  document.getElementById("f2").textContent=Math.floor(Math.random()*1e3);  
  document.getElementById("f3").textContent=Math.floor(Math.random()*500);  
